...
while True:
	...
	if int(p) == 1:
		...
		if(n == 2.1):
			...
			z=subprocess.getoutput("aws ec2 attach-volume --device /dev/sdb  --instance-id {} --volume-id {} ".format(instance_id,volume_id))
			print(z)
		elif(n==2.6):
			...
			z=subprocess.getoutput("aws s3api create-bucket --bucket {} --region {} --acl public-read --create-bucket-configuration LocationConstraint=ap-south-1 ".format(bname,region))
			print(z)
		elif(n==2.7):
			...
			z=subprocess.getoutput("aws  s3api list-buckets ")
			print(z)
		elif(n==2.13):
